...
export default class Vod extends Extension {
  async load() {
    this.registerSetting({
      ...
      type: 'input',
      defaultValue: '',
      description: '填写影视采集接口地址（AppleCMS / TVBox 直连 API，JSON/XML 均可），支持多行',
    });
    this.registerSetting({
      key: 'referer',
      title: '播放 Referer（可选）',
  ...
  async watch(url) {
    ...
    // 如果不是直链（不含 .m3u8/.mp4），说明是网页播放页，需要抓取真实 m3u8
    if (!/\.(m3u8|mp4|flv|ts)(\?|$)/i.test(playUrl)) {
      try {
        const html = String(await this._fetchText(playUrl));
        // 1) 从 HTML 里找 .m3u8 链接
        const m = html.match(/https?:\/\/[^"'\s<>]+\.m3u8[^"'\s<>]*/i);
        if (m) {
          playUrl = m[0];
        } else {
          // 2) 兜底：播放地址 + /index.m3u8
          playUrl = playUrl.replace(/\/+$/, '') + '/index.m3u8';
        }
      } catch (e) {
        // 抓取失败就按原地址播
      }
    ...
    const type = /\.m3u8(\?|$)/i.test(playUrl) ? 'hls' : 'mp4';
    return { type, url: playUrl, headers };
  }
}
